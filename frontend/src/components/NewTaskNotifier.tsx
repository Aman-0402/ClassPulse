  ...
  useEffect(() => {
    ...
    getStudentTasks()
      .then((tasks) => {
        ...
        const html =
          newTasks.length === 1
            ? `<strong>${escapeHtml(newTasks[0].title)}</strong>${newTasks[0].due_date ? `<br><span class="swal-task-due">Due ${escapeHtml(newTasks[0].due_date)}</span>` : ""}`
            : `<ul class="swal-task-list">${newTasks
                .map((t) => `<li>${escapeHtml(t.title)}${t.due_date ? ` (due ${escapeHtml(t.due_date)})` : ""}</li>`)
                .join("")}</ul>`;
