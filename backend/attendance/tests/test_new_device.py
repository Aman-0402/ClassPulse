...
class NewDeviceDetectionTest(APITestCase):
    def setUp(self):
        ...
        self.session1 = AttendanceSession.objects.create(teacher=self.teacher, subject="AI", section="A")
        self.qr1 = QRToken.objects.create(session=self.session1)
        self.session2 = AttendanceSession.objects.create(teacher=self.teacher, subject="AI-2", section="A")
        self.qr2 = QRToken.objects.create(session=self.session2)

    def test_new_device_logged_but_attendance_allowed_on_change(self):
    ...
    def test_same_phone_used_by_second_student_logged_but_allowed(self):
        other_student = User.objects.create_user(username="friend", password="pw12345678", role=User.ROLE_STUDENT)
        StudentProfile.objects.create(user=other_student, crn="102", course="BBA", semester=3, section="A")
